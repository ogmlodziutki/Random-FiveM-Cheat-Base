...
bool c_custom::sub_tab( const char* label, bool selected ) {
    ...
    draw->AddCircleFilled( ImVec2( rect.GetCenter( ).x, rect.Max.y + 10 ), 3 * value->second, settings::visuals::get_accent_color( value->second * GetStyle( ).Alpha ) );
...
void c_custom::tab_area( const char* str_id, ImVec2 size, std::function < void ( ) > content ) {
    ...
    {
        GetWindowDrawList( )->AddRectFilled( GetCurrentWindow( )->Pos, GetCurrentWindow( )->Pos + GetCurrentWindow( )->Size, ImColor( 15, 15, 15 ), GetStyle( ).WindowRounding, ImDrawFlags_RoundCornersTopLeft );
    }
    else
    {
        GetWindowDrawList( )->AddRectFilled( GetCurrentWindow( )->Pos, GetCurrentWindow( )->Pos + GetCurrentWindow( )->Size, ImColor( 233, 233, 233 ), GetStyle( ).WindowRounding, ImDrawFlags_RoundCornersTopLeft );
    }
...
void c_custom::begin_child( const char* name, ImVec2 size ) {
    ...
    if (settings::visuals::darkmode)
    {
        GetWindowDrawList( )->AddRectFilled( pos, pos + size, to_vec4( 15, 15, 15, custom.m_anim ), 4 );
    }
    else
    {
        GetWindowDrawList( )->AddRectFilled( pos, pos + size, to_vec4( 235, 235, 235, custom.m_anim ), 4 );
    }
    GetWindowDrawList( )->AddText( pos + ImVec2( 10, 8 ), settings::visuals::get_accent_color( custom.m_anim ), name, FindRenderedTextEnd( name ) );
